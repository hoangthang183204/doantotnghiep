...
@section('content')
    ...
    <div class="space-y-6 max-w-6xl mx-auto text-gray-900 dark:text-gray-100">
        ...
        {{-- ================= HEADER ================= --}}
        <div
            class="rounded-2xl p-6 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-sm text-gray-800 dark:text-white">
        ...
        <div
            class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-2 overflow-x-auto border border-gray-200 dark:border-slate-700">
            <nav class="flex items-center justify-between gap-1 min-w-[760px]" id="tabNav">
                <button
                    class="tab-btn active px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition whitespace-nowrap flex items-center gap-1.5"
                    data-tab="tab1">
                    📋 Thông tin
                </button>
                <button
                    class="tab-btn px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition whitespace-nowrap flex items-center gap-1.5"
                    data-tab="tab2">
                    💼 Công việc & HĐ
                </button>
                <button
        ...
        <div class="space-y-6">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="relative">
                                    ...
                                    @else
                                        <div
                                            class="relative rounded-xl border-2 border-dashed border-gray-300 aspect-[1.586/1] flex flex-col items-center justify-center">
                ...
                <div
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                        ...
                        @if ($hoSo?->hop_dong && $hoSo->hop_dong->count() > 0)
                            {{-- Sắp xếp hợp đồng tạo mới nhất lên đầu --}}
                            @php
                                $hopDongs = $hoSo->hop_dong->sortByDesc('created_at');
                            @endphp

                            <div class="space-y-3">
                                @foreach ($hopDongs as $item)
                                    @php
                                        // Logic xác định trạng thái hiệu lực / hết hạn
                                        $isExpired =
                                            $item->ngay_ket_thuc &&
                                            \Carbon\Carbon::parse($item->ngay_ket_thuc)->isPast();
                                        $isHetHan = $item->trang_thai_hop_dong == 'het_han' || $isExpired;
                                    @endphp

                                    <div
                                        class="bg-gray-50 dark:bg-slate-700 rounded-lg p-4 border-l-4 {{ $isHetHan ? 'border-red-500' : 'border-green-500' }}">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <span
                                                    class="font-medium text-gray-800 dark:text-white">{{ $item->ten_loai_hop_dong ?? $item->loai_hop_dong }}</span>
                                                <span
                                                ...
                                                <div class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                                    ...
                                                    →
                                                    {{ $item->ngay_ket_thuc ? \Carbon\Carbon::parse($item->ngay_ket_thuc)->format('d/m/Y') : 'Không xác định' }}
                                                </div>

                                                @if ($item->file_hop_dong_da_ky || $item->duong_dan_file)
                                                    <div class="mt-3 flex gap-2">
                                                        <button
                                                            onclick="openFilePreview('{{ asset('storage/' . ($item->file_hop_dong_da_ky ?? $item->duong_dan_file)) }}', 'Hợp đồng {{ $item->so_hop_dong }}')"
                                            ...
                                            @if ($isHetHan)
                                                <span
                                                    class="text-xs px-2.5 py-1 bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300 rounded-full font-medium whitespace-nowrap ml-2">
                                                    ❌ Hết hạn
                                                </span>
                                            @else
                                                <span
                                                    class="text-xs px-2.5 py-1 bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 rounded-full font-medium whitespace-nowrap ml-2">
                                                    ✅ Còn hiệu lực
                                                </span>
                                            @endif
                                        </div>
            ...
            <div id="tab3" class="tab-content hidden">
                <div
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex gap-2">
                                @if ($cv)
                                    <button
                                        onclick="openFilePreview('{{ asset('storage/' . ($cv->duong_dan_file ?? $cv->tep_tin)) }}', 'CV - {{ $user->ho_ten }}')"
                        ...
                        @if ($hoSo?->ky_nang && $hoSo->ky_nang->count() > 0)
                            <div class="flex flex-wrap gap-2">
                                @foreach ($hoSo->ky_nang as $item)
                                    <span
                                        class="px-3 py-1.5 bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 rounded-full text-sm font-medium shadow-sm">
                                        {{ $item->ten_ky_nang }}
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @foreach ($hoSo->chung_chi as $item)
                                    <div
                                        ...
                                        <p class="font-medium text-gray-800 dark:text-white">{{ $item->ten_chung_chi }}
                                        </p>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">🏛️ {{ $item->to_chuc_cap }}
                                        </p>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    {{-- Dự án đã tham gia (Phân trang 3 dự án / trang) --}}
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                        <h4 class="font-semibold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-1.5">
                            🚀 Dự án đã tham gia
                        </h4>
                        ...
                        @if ($hoSo?->du_an && $hoSo->du_an->count() > 0)
                            @php
                                // Sắp xếp dự án tạo mới nhất lên đầu
                                $duAns = $hoSo->du_an->sortByDesc('id')->values();
                            @endphp

                            {{-- Danh sách dự án --}}
                            <div id="project-list" class="space-y-3">
                                @foreach ($duAns as $index => $item)
                                    <div class="project-item bg-gray-50 dark:bg-slate-700 rounded-lg p-4 border-l-4 {{ $item->mau_border ?? 'border-blue-500' }} hover:shadow-md transition"
                                        data-page="{{ floor($index / 3) + 1 }}">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <span
                                            ...
                                            @if (!empty($item->trang_thai))
                                                <span
                                                    class="text-xs px-2 py-1 {{ $item->mau_trang_thai ?? 'bg-blue-100 text-blue-700' }} rounded-full">
                            ...
                            {{-- Thanh phân trang nếu có > 3 dự án --}}
                            @if ($hoSo->du_an->count() > 3)
                                <div
                                    class="mt-4 flex items-center justify-between border-t border-gray-100 dark:border-slate-700 pt-3 text-xs">
                                    <span class="text-gray-500 dark:text-gray-400">
                                        Hiển thị <span id="project-range">1-3</span> / {{ $hoSo->du_an->count() }} dự án
                                    </span>
                                    <div class="flex items-center gap-1" id="project-pagination-btns">
                                        <button onclick="changeProjectPage(-1)" id="btn-prev-project"
                                            class="px-2.5 py-1 rounded bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-gray-200 disabled:opacity-40">
                                            ❮ Trước
                                        </button>
                                        <span id="project-page-numbers" class="flex gap-1"></span>
                                        <button onclick="changeProjectPage(1)" id="btn-next-project"
                                            class="px-2.5 py-1 rounded bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-gray-200 disabled:opacity-40">
                                            Sau ❯
                                        </button>
                                    </div>
            ...
            <div id="tab4" class="tab-content hidden">
                <div
                    ...
                    <h3
                        class="text-lg font-semibold text-gray-800 dark:text-white border-b border-gray-200 dark:border-slate-700 pb-3 mb-4 flex items-center gap-2">
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <span
                                class="ml-auto text-xs px-3 py-1 bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 rounded-full font-medium">
                                Chi trả hàng tháng
                            </span>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div
                            class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4 text-center border border-gray-200 dark:border-slate-600">
                        ...
                        @if ($phuCapChiTiets->count() > 0)
                            <span>Phụ cấp trong HĐ hiện tại:
                                @foreach ($phuCapChiTiets as $pc)
                                    <strong>{{ $pc->ten }} ({{ number_format($pc->so_tien_mac_dinh, 0, ',', '.') }}
                                        ₫)</strong>{{ !$loop->last ? ', ' : '' }}
                                @endforeach
                            </span>
                        @else
                    ...
                    <div class="mb-6">
                        ...
                        <div
                            class="inline-block px-3 py-1.5 bg-amber-50 dark:bg-amber-900/20 rounded-full border border-amber-200 dark:border-amber-800 text-sm">
                            <span class="text-amber-700 dark:text-amber-400 font-medium">Tiền tăng ca:</span>
                            <strong
                                class="text-amber-700 dark:text-amber-400 font-bold ml-1">{{ number_format($tienTangCa, 0, ',', '.') }}
                                ₫</strong>
                        </div>
                    </div>
            ...
            <div id="tab5" class="tab-content hidden">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between py-2 border-b border-gray-100 dark:border-slate-700">
                                ...
                                <span
                                    class="font-bold text-green-600 dark:text-green-400">{{ number_format($luongDongBhxh, 0, ',', '.') }}
                                    VNĐ</span>
                            </div>
                            ...
                            <div class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4 mt-3 space-y-2">
                                <div class="flex justify-between text-sm py-1">
                                    ...
                                    <span
                                        class="font-semibold text-blue-600 dark:text-blue-400">{{ number_format($bhxh, 0, ',', '.') }}
                                        VNĐ</span>
                                </div>
                                <div
                                    class="flex justify-between text-sm py-1 border-t border-gray-200 dark:border-slate-600">
                                    <span class="text-gray-500 dark:text-gray-400">BHYT (1.5%)</span>
                                    <span
                                        class="font-semibold text-blue-600 dark:text-blue-400">{{ number_format($bhyt, 0, ',', '.') }}
                                        VNĐ</span>
                                </div>
                                <div
                                    class="flex justify-between text-sm py-1 border-t border-gray-200 dark:border-slate-600">
                                    ...
                                    <span
                                        class="font-semibold text-blue-600 dark:text-blue-400">{{ number_format($bhtn, 0, ',', '.') }}
                                        VNĐ</span>
                                </div>
                                <div
                                    class="flex justify-between text-sm py-2 border-t-2 border-gray-300 dark:border-slate-500 font-bold mt-1">
                                    <span class="text-gray-700 dark:text-gray-200">Tổng đóng (10.5%)</span>
                                    <span
                                        class="text-red-600 dark:text-red-400">{{ number_format($tongBaoHiem, 0, ',', '.') }}
                                        VNĐ</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        class="bg-blue-50/50 dark:bg-blue-900/10 rounded-xl shadow-sm p-6 border border-blue-100 dark:border-blue-900/30">
                        <h3
                            class="text-lg font-semibold text-gray-800 dark:text-white border-b border-blue-200 dark:border-blue-800 pb-3 mb-4 flex items-center gap-2">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between py-1">
                                ...
                                <span
                                    class="font-semibold text-gray-800 dark:text-gray-200">{{ number_format($tongThuNhap, 0, ',', '.') }}
                                    ₫</span>
                            </div>
                            ...
                            <div class="flex justify-between py-1 border-t border-blue-100 dark:border-blue-900/20">
                                ...
                                <span class="font-semibold text-red-600">-{{ number_format($tongBaoHiem, 0, ',', '.') }}
                                    ₫</span>
                            </div>

                            <div
                                class="flex justify-between py-1 border-t border-blue-100 dark:border-blue-900/20 font-medium">
                            ...
                            <div
                                class="flex justify-between py-2 border-t-2 border-blue-200 dark:border-blue-800 mt-2 font-bold text-base">
                                <span class="text-gray-800 dark:text-white flex items-center gap-1.5">
                                    <span>🏛️</span> Thuế TNCN
                                </span>
                                <span
                                    class="text-emerald-600 dark:text-emerald-400">{{ number_format($thueTncn, 0, ',', '.') }}
                                    ₫</span>
                            </div>

                            <div
                                class="flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-medium mt-1">
                                <i class="fa-regular fa-circle-check"></i> Không phải nộp thuế
                            </div>

                            {{-- Khối Thực Nhận Màu Xanh Lá Nổi Bật --}}
                            <div
                                class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border-2 border-green-300 dark:border-green-800 mt-4 shadow-sm">
                                <div class="flex justify-between items-center">
                                    <span
                                        class="text-green-900 dark:text-green-300 font-bold text-base flex items-center gap-2">
                                        <span>💰</span> THỰC NHẬN
                                    </span>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            @if ($hoSo?->dao_tao && $hoSo->dao_tao->count() > 0)
                                <div class="space-y-3">
                                    @foreach ($hoSo->dao_tao as $item)
                                        <div
                                            class="bg-blue-50/40 dark:bg-blue-900/10 rounded-xl p-4 border-l-4 border-blue-500 hover:shadow-md transition">
                                            <div class="flex justify-between items-start">
                                                <div>
                                                    <span
                                                        class="font-bold text-gray-800 dark:text-white text-base">{{ $item->ten_khoa_hoc }}</span>
                                                    @if ($item->to_chuc)
                                                        <span
                                                            class="text-sm text-gray-500 dark:text-gray-400 ml-1">({{ $item->to_chuc }})</span>
                                                    @endif
                                                </div>
                                                @if ($item->co_chung_chi)
                                                    <span
                                                        class="text-xs px-2.5 py-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 rounded-full font-semibold flex items-center gap-1">
                                                        📜 Có chứng chỉ
                                                    </span>
                                                @endif
                                            </div>
                                            ...
                                            <div
                                                class="text-xs text-indigo-600 dark:text-indigo-400 mt-2 font-medium flex items-center gap-1">
                                            ...
                                            @if ($item->ket_qua)
                                                <div
                                                    ...
                                                    <i class="fa-solid fa-circle-check"></i> Kết quả:
                                                    {{ $item->ket_qua }}
                                                </div>
                                            @endif
                                        </div>
                        ...
                        <div
                                        ...
                                        <div class="rounded-xl p-4 {{ $item->mau_loai }}">
                                            <div class="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    @if ($item->so_tien)
                                                        <span
                                                            class="text-sm font-semibold {{ $item->loai == 'khen_thuong' ? 'text-green-600' : 'text-red-600' }} ml-2">
                                                ...
                                                <span
                                                    class="text-xs text-gray-500 dark:text-gray-400 font-medium">{{ $item->ngay?->format('d/m/Y') }}</span>
                                            </div>
                                            @if ($item->noi_dung)
                            ...
                            @else
                                <div
                                    class="text-center py-12 bg-gray-50/50 dark:bg-slate-700/30 rounded-2xl flex flex-col items-center justify-center">
                                    <div
                                        class="w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-500 flex items-center justify-center text-xl mb-3">
                                        ⚖️
                                    </div>
                                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium">Chưa có khen thưởng
                                        hoặc kỷ luật</p>
                                </div>
                            @endif
                        </div>
                    ...
                    <div
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div
                                class="bg-blue-50/60 dark:bg-blue-900/20 rounded-xl p-4 text-center border border-blue-100 dark:border-blue-900/30">
                            ...
                            <div
                                class="bg-green-50/60 dark:bg-green-900/20 rounded-xl p-4 text-center border border-green-100 dark:border-green-900/30">
                            ...
                            <div
                                class="bg-red-50/60 dark:bg-red-900/20 rounded-xl p-4 text-center border border-red-100 dark:border-red-900/30">
                            ...
                            <div
                                class="bg-purple-50/60 dark:bg-purple-900/20 rounded-xl p-4 text-center border border-purple-100 dark:border-purple-900/30">
                                <div class="text-3xl font-bold text-purple-600 dark:text-purple-400">
                                    {{ $hoSo?->nguoiPhuThuoc?->count() ?? 0 }}
                                </div>
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 font-medium">Người phụ thuộc
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    @foreach ($lichSuNghiPhep as $item)
                                        ...
                                        <tr
                                            ...
                                            <td class="p-2.5 text-xs text-center font-medium">
                                                {{ number_format($item->so_ngay_nghi, 1) }}</td>
                                            <td class="p-2.5 text-xs text-center font-medium text-orange-600">
                                                {{ number_format($soNgayDaDung, 1) }}</td>
                                            <td
                                                class="p-2.5 text-xs text-center font-bold {{ $conLai <= 0 ? 'text-red-600' : 'text-green-600' }}">
                                                {{ number_format($conLai, 1) }}</td>
                                ...
                                <tbody>
                                    @foreach ($lichSuTangCa as $item)
                                        ...
                                        <tr
                                            ...
                                            <td class="p-2.5 text-xs">
                                                @if ($item->nguoi_duyet)
                                                    {{ $item->nguoi_duyet->hoSo->ho ?? '' }}
                                                    {{ $item->nguoi_duyet->hoSo->ten ?? '' }}
                                                    <br><span
                                    ...
                                    @foreach ($lichSuVeSom as $item)
                                        ...
                                        <tr
                                            ...
                                            <td class="p-2.5 text-xs">
                                                ...
                                                @endphp
                                                @if ($nguoiDuyetModel)
                                                    {{ $nguoiDuyetModel->hoSo->ho ?? '' }}
                                                    {{ $nguoiDuyetModel->hoSo->ten ?? '' }}
                                                    <br><span
                                                        class="text-gray-400 text-[10px]">{{ $item->thoi_gian_duyet ? \Carbon\Carbon::parse($item->thoi_gian_duyet)->format('d/m/Y H:i') : '' }}</span>
                                                @else
                                                    <span class="text-gray-400">---</span>
    ...
    <script>
        ...
        function renderProjectPagination() {
            ...
            const totalPages = Math.ceil(items.length / projectsPerPage);

            // Hiển thị/ẩn dự án theo trang
            items.forEach(item => {
            ...
            const start = (currentProjectPage - 1) * projectsPerPage + 1;
            const end = Math.min(currentProjectPage * projectsPerPage, items.length);
            const rangeEl = document.getElementById('project-range');
            ...
            if (pageNumbersContainer) {
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    btn.textContent = i;
                    btn.className = `px-2.5 py-1 rounded text-xs font-semibold ${
                i === currentProjectPage 
