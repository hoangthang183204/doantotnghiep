    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Quản lý đơn tăng ca của nhân viên trong phòng
            ...
            <div class="flex items-center gap-2">
                <span class="text-sm bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-lg text-gray-600 dark:text-gray-300">
                    <i class="fas fa-clock mr-1"></i> {{ $thongKe['tong'] }} đơn
                </span>
                @if ($thongKe['cho_duyet'] > 0)
                    <span
                        class="text-sm bg-yellow-100 dark:bg-yellow-900/30 px-3 py-1.5 rounded-lg text-yellow-700 dark:text-yellow-400">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
            <form method="GET" class="flex flex-wrap items-center gap-3">
                ...
                {{-- ⭐ SỬA: dùng route duyet-tang-ca.index --}}
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <div
                                            ...
                                            {{ substr($item->nguoi_dung->hoSo->ten ?? $item->nguoi_dung->ten_dang_nhap, 0, 1) }}
                                        </div>
                                        <div>
                                            <p class="font-medium text-gray-800 dark:text-white">
                                                {{ $item->nguoi_dung->hoSo->ho ?? '' }}
                                                {{ $item->nguoi_dung->hoSo->ten ?? '' }}
                                            </p>
                                            <p class="text-xs text-gray-400">
                                                {{ $item->nguoi_dung->hoSo->ma_nhan_vien ?? 'N/A' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">{{ \Carbon\Carbon::parse($item->ngay_tang_ca)->format('d/m/Y') }}
                                </td>
                                <td class="px-4 py-3">{{ $item->gio_bat_dau }} - {{ $item->gio_ket_thuc }}</td>
                                <td class="px-4 py-3 text-center font-medium">{{ $item->so_gio_tang_ca }}h</td>
                                <td class="px-4 py-3">
                                    ...
                                    @elseif($item->trang_thai == 'tu_choi')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">
                                            <span class="w-1.5 h-1.5 bg-red-500 rounded-full mr-1.5"></span>
                                            Từ chối
                                        </span>
                                        {{-- 🔴 THÊM TRƯỜNG HỢP 'huy_bo' ĐỂ HIỂN THỊ THÀNH 'Đã hủy' --}}
                                    @elseif($item->trang_thai == 'huy')
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-600">
                                            Đã hủy
                                        </span>
                                    @else
                                        <span
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        {{-- ⭐ SỬA: dùng route duyet-tang-ca.show --}}
                                        <a href="{{ route('duyet-tang-ca.show', $item->id) }}"
                            ...
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="font-medium">Không có đơn tăng ca nào</p>
    ...
    @push('scripts')
        <script>
            function duyetTangCa(id) {
                ...
                {{-- ⭐ SỬA: dùng route duyet-tang-ca.duyet --}}
            ...
            function tuChoiTangCa(id) {
                ...
                {{-- ⭐ SỬA: dùng route duyet-tang-ca.tu-choi --}}
            ...
            function showToast(message, type = 'success') {
                ...
                toast.className =
                    `fixed top-4 right-4 ${colors[type] || 'bg-blue-500'} text-white px-6 py-3 rounded-xl shadow-lg z-50 transition-all duration-300`;
