    ...
    <div class="bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5">
        <form method="GET" action="{{ route('admin.thuc-hien-tang-ca.index') }}">
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <select name="trang_thai" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="chua_lam" {{ request('trang_thai') == 'chua_lam' ? 'selected' : '' }}>🟡 Chưa làm</option>
                        ...
                        <option value="khong_hoan_thanh" {{ request('trang_thai') == 'khong_hoan_thanh' ? 'selected' : '' }}>🔴 Không hoàn thành</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="thang" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                ...
                <div>
                    ...
                    <select name="nam" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
            ...
            <div class="mt-5 flex gap-3 flex-wrap">
                ...
                <a href="{{ route('admin.thuc-hien-tang-ca.index') }}" class="px-5 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full">
                ...
                <tbody>
                    @forelse($danhSachTangCa ?? [] as $item)
                        @php
                            $dangKy = $item->dangKyTangCa;
                            $hoTen = optional($dangKy->nguoiDung->hoSo)
                                ? $dangKy->nguoiDung->hoSo->ho . ' ' . $dangKy->nguoiDung->hoSo->ten
                                : $dangKy->nguoiDung->ten_dang_nhap ?? 'N/A';
                            $avatar = optional($dangKy->nguoiDung->hoSo)->anh_dai_dien 
                                ? asset($dangKy->nguoiDung->hoSo->anh_dai_dien) 
                                : asset('assets/images/default.png');
                            
                            $badgeClass = match($item->trang_thai) {
                                'chua_lam' => 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300',
                        ...
                        <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            Mã NV: {{ optional($dangKy->nguoiDung->hoSo)->ma_nhan_vien ?? 'N/A' }}
                                        </div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            Phòng: {{ optional($dangKy->nguoiDung->phongBan)->ten_phong_ban ?? 'N/A' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-700 dark:text-gray-300">
                                    {{ \Carbon\Carbon::parse($dangKy->ngay_tang_ca)->format('d/m/Y') }}
                                </div>
                                <div class="text-xs text-gray-400">
                                    {{ \Carbon\Carbon::parse($dangKy->ngay_tang_ca)->locale('vi')->dayName }}
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="space-y-1">
                                    <span class="px-2 py-1 rounded text-xs bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300">
                                        Vào: {{ $item->gio_bat_dau_thuc_te ? substr($item->gio_bat_dau_thuc_te, 0, 5) : '--:--' }}
                                    </span>
                                    <br>
                                    <span class="px-2 py-1 rounded text-xs bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">
                                        Ra: {{ $item->gio_ket_thuc_thuc_te ? substr($item->gio_ket_thuc_thuc_te, 0, 5) : '--:--' }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <span class="font-semibold text-blue-600 dark:text-blue-400">
                                    {{ number_format($item->so_gio_tang_ca_thuc_te, 1) }}h
                                </span>
                            </td>
                            <td class="px-4 py-3">
                                <span class="font-semibold text-green-600 dark:text-green-400">
                                    {{ number_format($item->so_cong_tang_ca, 2) }}
                                </span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('admin.thuc-hien-tang-ca.show', $item->id) }}"
                                        class="px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-800 text-blue-600 dark:text-blue-300 rounded-lg text-xs transition flex items-center gap-1">
                                    ...
                                    @php
                                        $user = auth()->user();
                                        $canEdit = $user->coVaiTro('Admin') || $user->coVaiTro('HR');
                                    @endphp
                                    @if($canEdit)
                                    <a href="{{ route('admin.thuc-hien-tang-ca.edit', $item->id) }}"
                                        class="px-3 py-1.5 bg-yellow-50 dark:bg-yellow-900/30 hover:bg-yellow-100 dark:hover:bg-yellow-800 text-yellow-700 dark:text-yellow-300 rounded-lg text-xs transition flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
