    ...
    <div class="space-y-6">
                ...
                <div>
                    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-1">
                        <a href="{{ route('admin.dashboard') }}"
                            class="hover:text-gray-700 dark:hover:text-gray-300">Dashboard</a>
                        <span>/</span>
                        <a href="{{ route('admin.tang-ca.index') }}" class="hover:text-gray-700 dark:hover:text-gray-300">Tăng
                            ca</a>
                        <span>/</span>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Xem thông tin và xét duyệt đơn đăng ký tăng
                        ca</p>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="flex items-center gap-4">
                            ...
                            @endphp
                            <div
                                class="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center text-gray-600 dark:text-gray-300 font-bold text-lg">
                                {{ $initial }}
                            </div>
                            <div>
                                <h3 class="font-semibold text-gray-800 dark:text-white">{{ $hoTen }}</h3>
                                <p class="text-xs text-gray-500">Mã NV:
                                    {{ optional($tangCa->nguoi_dung->hoSo)->ma_nhan_vien ?? 'N/A' }}</p>
                                <p class="text-xs text-gray-500">Phòng ban:
                                    {{ optional($tangCa->nguoi_dung->phongBan)->ten_phong_ban ?? 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div>
                                ...
                                <p class="font-medium text-gray-800 dark:text-white">
                                    {{ $tangCa->ngay_tang_ca->format('d/m/Y') }}</p>
                                <p class="text-xs text-gray-400">
                                    {{ ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'][$tangCa->ngay_tang_ca->dayOfWeek] }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500">Thời gian</p>
                                <p class="font-medium text-gray-800 dark:text-white">
                                    {{ \Carbon\Carbon::parse($tangCa->gio_bat_dau)->format('H:i') }} -
                                    {{ \Carbon\Carbon::parse($tangCa->gio_ket_thuc)->format('H:i') }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500">Số giờ</p>
                                <p class="font-semibold text-blue-600 dark:text-blue-400 text-lg">
                                    {{ number_format($tangCa->so_gio_tang_ca, 1) }}<span class="text-sm"> giờ</span></p>
                            </div>
                            <div>
                                ...
                                @endphp
                                <span
                                    class="px-2 py-0.5 rounded-full text-xs font-medium {{ $loaiColors[$tangCa->loai_tang_ca] ?? 'bg-gray-100' }}">
                        ...
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    <p class="font-medium text-gray-800 dark:text-white">
                                        {{ \Carbon\Carbon::parse($th->gio_bat_dau_thuc_te)->format('H:i') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Giờ kết thúc</p>
                                    <p class="font-medium text-gray-800 dark:text-white">
                                        {{ \Carbon\Carbon::parse($th->gio_ket_thuc_thuc_te)->format('H:i') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Số giờ thực tế</p>
                                    <p class="font-semibold text-blue-600 dark:text-blue-400">
                                        {{ number_format($th->so_gio_tang_ca_thuc_te, 1) }} giờ</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Công tăng ca</p>
                                    <p class="font-semibold text-green-600 dark:text-green-400">{{ $th->so_cong_tang_ca }}
                                    </p>
                                </div>
                            </div>
                            @if ($th->cong_viec_da_thuc_hien)
                                <div class="mt-4 bg-gray-50 dark:bg-gray-700/30 rounded-lg p-3">
                                    ...
                                    <p class="text-sm text-gray-700 dark:text-gray-300">{{ $th->cong_viec_da_thuc_hien }}
                                    </p>
                                </div>
                            @endif
                            @if ($th->ghi_chu)
                            ...
                            @if ($tangCa->luong_tang_ca)
                                <div
                                    class="mt-3 bg-green-50 dark:bg-green-900/20 rounded-lg p-3 border border-green-200 dark:border-green-800">
                                    ...
                                    <p class="text-lg font-bold text-green-700 dark:text-green-300">
                                        {{ number_format($tangCa->luong_tang_ca, 0) }}đ</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                        ...
                        @if ($tangCa->nguoi_duyet)
                            <div class="mt-4 space-y-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span
                                        class="text-gray-700 dark:text-gray-300">{{ $tangCa->thoi_gian_duyet?->format('d/m/Y H:i') }}</span>
                                </div>
                            ...
                            <div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                                ...
                                @php
                                    $thucHienStatus = [
                                        ...
                                        'quan_ly_xac_nhan' => [
                                            'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
                                            '✅ Quản lý đã xác nhận hoàn thành',
                                        ],
                                    ];
                                    $thConfig = $thucHienStatus[$tangCa->thuc_hien->trang_thai] ?? [
                                        'bg-gray-100 text-gray-700',
                                        $tangCa->thuc_hien->trang_thai,
                                    ];
                                @endphp
                ...
                {{-- ACTION BUTTONS - CHỈ HIỂN THỊ KHI ĐƠN CHỜ DUYỆT --}}
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <form method="POST" action="{{ route('admin.tang-ca.duyet', $tangCa->id) }}"
                                onsubmit="return confirm('Xác nhận phê duyệt đơn tăng ca này?')">
                                @csrf
                                <button type="submit"
                            ...
                            {{-- Form Từ chối --}}
                            <form method="POST" action="{{ route('admin.tang-ca.tu-choi', $tangCa->id) }}"
                                onsubmit="return xacNhanTuChoi()">
                                @csrf
                                <div class="mb-3">
                                ...
                                <button type="submit"
                                    class="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg transition flex items-center justify-center gap-2 text-sm font-medium">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if ($tangCa->trang_thai === 'da_duyet' && $tangCa->thuc_hien && $tangCa->thuc_hien->trang_thai === 'nhan_vien_xac_nhan')
                    <div
                        class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden border-2 border-green-200 dark:border-green-800">
                        <div
                            class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-green-50 dark:bg-green-900/20">
                ...
                {{-- HIỂN THỊ THÔNG BÁO NẾU ĐÃ DUYỆT HOẶC TỪ CHỐI --}}
                @if ($tangCa->trang_thai === 'da_duyet' && !$tangCa->thuc_hien)
                    <div
                        class="bg-blue-50 dark:bg-blue-900/20 rounded-xl shadow-sm overflow-hidden border border-blue-200 dark:border-blue-800">
                        <div class="px-6 py-4">
                            <p class="text-sm text-blue-600 dark:text-blue-400 text-center">
                                <i class="fas fa-info-circle mr-2"></i>
                                Đơn đã được duyệt. Đang chờ nhân viên xác nhận đã làm tăng ca.
                            </p>
                        </div>
                    </div>
                @endif

                @if ($tangCa->trang_thai === 'tu_choi')
                    <div
                        class="bg-red-50 dark:bg-red-900/20 rounded-xl shadow-sm overflow-hidden border border-red-200 dark:border-red-800">
                        <div class="px-6 py-4">
                ...
                @if ($tangCa->trang_thai === 'huy')
                    <div
                        class="bg-gray-50 dark:bg-gray-700/30 rounded-xl shadow-sm overflow-hidden border border-gray-200 dark:border-gray-700">
                        <div class="px-6 py-4">
    ...
    @push('scripts')
        <script>
            function xacNhanTuChoi() {
                const lyDo = document.querySelector('#form-tu-choi textarea[name="ly_do_tu_choi"]').value.trim();
                if (!lyDo) {
