...
@section('title', 'Thêm khấu trừ khác')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Thêm khoản khấu trừ khác</h1>
        <a href="{{ route('admin.khau-tru-khac.index', ['thang' => $thang, 'nam' => $nam]) }}"
    ...
    <form action="{{ route('admin.khau-tru-khac.store') }}" method="POST"
        ...
        <div>
            ...
            <select name="nguoi_dung_id" required
                ...
                @foreach($nhanViens as $nv)
                    <option value="{{ $nv->id }}" @selected(old('nguoi_dung_id') == $nv->id)>
                        {{ trim(($nv->ho_so->ho ?? '') . ' ' . ($nv->ho_so->ten ?? '')) ?: $nv->ten_dang_nhap }}
                    </option>
                @endforeach
            </select>
            @error('nguoi_dung_id') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>
            ...
            <div>
                ...
                <select name="loai" class="w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white">
                    @foreach(\App\Models\KhauTruKhac::$loaiLabels as $key => $label)
                        <option value="{{ $key }}" @selected(old('loai') === $key)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <input type="number" name="so_tien" value="{{ old('so_tien') }}" min="0" step="1000" required
                       class="w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white">
                @error('so_tien') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="ly_do" value="{{ old('ly_do') }}" maxlength="255"
                   placeholder="VD: Tạm ứng lương, phạt đi trễ..."
                   class="w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-900 dark:text-white">
        </div>

        <input type="hidden" name="trang_thai" value="hieu_luc">
