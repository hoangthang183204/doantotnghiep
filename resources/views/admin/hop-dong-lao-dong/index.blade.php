...
@section('content')
    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.hop-dong.luu-tru') }}"
                        class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gray-600 hover:bg-gray-700 text-white rounded-xl transition shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if (session('success'))
            <div
                class="bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-300 px-4 py-3 rounded-xl flex justify-between items-center">
        ...
        @if (session('error'))
            <div
                class="bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-3 rounded-xl flex justify-between items-center">
                <div class="flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5">
            <form method="GET" action="{{ route('admin.hop-dong.index') }}">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="loai_hop_dong"
                            ...
                            <option value="thu_viec" {{ request('loai_hop_dong') == 'thu_viec' ? 'selected' : '' }}>Thử việc
                            </option>
                            ...
                            <option value="mua_vu" {{ request('loai_hop_dong') == 'mua_vu' ? 'selected' : '' }}>Mùa vụ
                            </option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">📊 Trạng thái hợp
                            đồng</label>
                        <select name="trang_thai_hop_dong"
                            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 transition">
                            <option value="">-- Tất cả --</option>
                            <option value="tao_moi" {{ request('trang_thai_hop_dong') == 'tao_moi' ? 'selected' : '' }}>🆕
                                Tạo mới</option>
                            <option value="hieu_luc" {{ request('trang_thai_hop_dong') == 'hieu_luc' ? 'selected' : '' }}>✅
                                Hiệu lực</option>
                            <option value="chua_hieu_luc"
                                {{ request('trang_thai_hop_dong') == 'chua_hieu_luc' ? 'selected' : '' }}>⏳ Chưa hiệu lực
                            </option>
                            <option value="het_han" {{ request('trang_thai_hop_dong') == 'het_han' ? 'selected' : '' }}>⏰
                                Hết hạn</option>
                            <option value="huy_bo" {{ request('trang_thai_hop_dong') == 'huy_bo' ? 'selected' : '' }}>❌ Hủy
                                bỏ</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">✍️ Trạng thái
                            ký</label>
                        <select name="trang_thai_ky"
                            class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 transition">
                            <option value="">-- Tất cả --</option>
                            <option value="cho_ky" {{ request('trang_thai_ky') == 'cho_ky' ? 'selected' : '' }}>⏳ Chờ ký
                            </option>
                ...
                <div class="mt-5 flex gap-3">
                    <button type="submit"
                        class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition flex items-center gap-2 shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.hop-dong.index') }}"
                        class="px-5 py-2.5 bg-gray-500 hover:bg-gray-600 text-white rounded-xl transition flex items-center gap-2 shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @if ($hopDongTaoMoi > 0)
                <div
                    class="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700 flex items-center gap-3">
                    ...
                    <div>
                        <span class="font-semibold">{{ $hopDongTaoMoi }}</span> hợp đồng ở trạng thái <strong>"Tạo
                            mới"</strong> cần gửi cho nhân viên.
                    </div>
                </div>
            @endif
            ...
            @if ($hopDongSapHetHan > 0)
                <div
                    class="p-4 rounded-xl bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-700 flex items-center gap-3">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-medium text-gray-800 dark:text-white">{{ $hoTen ?: 'N/A' }}
                                            </div>
                                            <div class="text-xs text-gray-500">Mã:
                                                {{ optional($hd->hoSoNguoiDung)->ma_nhan_vien ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-semibold {{ match ($hd->loai_hop_dong) {
                                ...
                                <td class="px-4 py-3 font-semibold text-green-600">
                                    {{ number_format($hd->luong_co_ban, 0, ',', '.') }}đ</td>
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-semibold {{ match ($hd->trang_thai_ky) {
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 rounded-full text-xs font-semibold {{ match ($hd->trang_thai_hop_dong) {
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex justify-center gap-1.5 flex-wrap">
                                        ...
                                        {{-- ⭐ NÚT TĂNG LƯƠNG - CHỈ HIỆN KHI HỢP ĐỒNG CÓ HIỆU LỰC VÀ ĐÃ KÝ --}}
                                        @if ($hd->trang_thai_hop_dong == 'hieu_luc' && $hd->trang_thai_ky == 'da_ky')
                                            <a href="{{ route('admin.tang-luong.create', $hd->id) }}"
                                        ...
                                        {{-- ⭐ NÚT LƯU TRỮ - CHỈ HIỆN KHI HỢP ĐỒNG HẾT HẠN VÀ CHƯA TÁI KÝ --}}
                                        @if ($hd->trang_thai_hop_dong == 'het_han' && $hd->trang_thai_tai_ky != 'da_tai_ky')
                                            <form action="{{ route('admin.hop-dong.an-khoi-danh-sach') }}" method="POST"
                                                class="inline"
                                                onsubmit="return confirm('📦 Bạn có chắc muốn chuyển hợp đồng {{ $hd->so_hop_dong }} vào lưu trữ?')">
                                                @csrf
                                                <input type="hidden" name="hop_dong_id" value="{{ $hd->id }}">
                                                <button type="submit"
                                                    class="p-1.5 text-gray-600 hover:bg-gray-50 rounded-lg transition"
                                                    title="📦 Lưu trữ">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        @if (in_array($hd->trang_thai_hop_dong, ['tao_moi', 'het_han', 'huy_bo']))
