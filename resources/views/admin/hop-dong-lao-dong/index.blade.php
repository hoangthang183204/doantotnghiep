...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800/80 backdrop-blur border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm p-6">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">📄 Quản lý hợp đồng lao động</h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">Danh sách hợp đồng lao động trong hệ thống</p>
            </div>
            <div class="flex gap-3 flex-wrap">
                ...
                <a href="{{ route('admin.hop-dong.thong-ke') }}"
                    class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition shadow-md hover:shadow-lg">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5">
        <form method="GET" action="{{ route('admin.hop-dong.index') }}">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">📊 Trạng thái HĐ</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">🔍 Trạng thái duyệt</label>
            ...
            <div class="mt-5 flex gap-3 flex-wrap">
                ...
                <a href="{{ route('admin.hop-dong.export', request()->all()) }}"
                    class="px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl transition flex items-center gap-2 shadow-md hover:shadow-lg">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($hopDongs as $index => $hd)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                            ...
                            <td class="px-4 py-3">
                                @switch($hd->trang_thai_duyet)
                                    @case('cho_duyet')
                                        <span class="px-2 py-1 rounded-full text-xs bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300">⏳ Chờ duyệt</span>
                                        @break
                                    @case('da_duyet')
                                        <span class="px-2 py-1 rounded-full text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">✅ Đã duyệt</span>
                                        @break
                                    @case('tu_choi')
                                        <span class="px-2 py-1 rounded-full text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300">❌ Từ chối</span>
                                        @break
                                    @default
                            ...
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-1.5 flex-wrap">
                                    ...
                                    <a href="{{ route('admin.hop-dong.show', $hd->id) }}"
                                        class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Xem">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($hd->trang_thai_duyet === 'cho_duyet' && $isAdmin)
                                        <form action="{{ route('admin.hop-dong.duyet', $hd->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit"
                                                ...
                                                onclick="return confirm('✅ Xác nhận duyệt hợp đồng {{ $hd->so_hop_dong }}?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($hd->trang_thai_duyet === 'da_duyet' && $hd->trang_thai_ky === 'cho_ky' && ($isAdmin || $isHr))
                                        <form action="{{ route('admin.hop-dong.gui-ky', $hd->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit"
                                                ...
                                                title="📨 Gửi cho nhân viên ký"
                                                onclick="return confirm('📨 Gửi hợp đồng {{ $hd->so_hop_dong }} cho nhân viên ký?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {{-- Nút Tăng lương - Chỉ hiện khi hợp đồng hiệu lực và đã ký --}}
                                    @if($hd->trang_thai_hop_dong == 'hieu_luc' && $hd->trang_thai_ky == 'da_ky')
                                        <a href="{{ route('admin.tang-luong.create', $hd->id) }}"
                                            class="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition" title="💰 Tăng lương">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    {{-- Nút Xóa - Chỉ hiện khi ở trạng thái tao_moi, het_han, huy_bo --}}
                                    @if(in_array($hd->trang_thai_hop_dong, ['tao_moi', 'het_han', 'huy_bo']) && $isAdmin)
                                        <form action="{{ route('admin.hop-dong.destroy', $hd->id) }}" method="POST"
                                            onsubmit="return confirm('🗑️ Bạn có chắc muốn xóa hợp đồng {{ $hd->so_hop_dong }}?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
