                ...
                <div class="p-6">
                    <div class="text-center py-6">
                        @if(!$chamCongHomNay || !$chamCongHomNay->gio_vao)
                            <!-- Chưa check-in -->
                            <div class="w-24 h-24 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center mx-auto mb-4">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Hôm nay: {{ Carbon\Carbon::now()->format('d/m/Y') }}</p>
                        ...
                        @elseif($chamCongHomNay->gio_vao && !$chamCongHomNay->gio_ra)
                            <!-- Đã check-in -->
                            <div class="w-24 h-24 rounded-full bg-yellow-50 dark:bg-yellow-900/20 flex items-center justify-center mx-auto mb-4">
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Check-in lúc: <strong class="text-gray-900 dark:text-white">{{ $chamCongHomNay->gio_vao_format }}</strong>
                            </p>
                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">
                                Phương thức: <span class="font-medium">{{ $chamCongHomNay->phuong_thuc_cham_cong_text }}</span>
                            </p>
                        ...
                        @else
                            <!-- Đã hoàn thành -->
                            <div class="w-24 h-24 rounded-full bg-green-50 dark:bg-green-900/20 flex items-center justify-center mx-auto mb-4">
                            ...
                            <div class="grid grid-cols-2 gap-4 max-w-md mx-auto mt-3">
                                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                                    ...
                                    <p class="font-semibold text-gray-900 dark:text-white">{{ $chamCongHomNay->gio_vao_format }}</p>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Check-out</p>
                                    <p class="font-semibold text-gray-900 dark:text-white">{{ $chamCongHomNay->gio_ra_format }}</p>
                                </div>
                            </div>
                            <div class="mt-3 flex items-center justify-center space-x-6">
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Số giờ làm</p>
                                    <p class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ $chamCongHomNay->so_gio_lam }} giờ</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Tăng ca</p>
                                    <p class="text-xl font-bold text-green-600 dark:text-green-400">{{ $chamCongHomNay->gio_tang_ca }} giờ</p>
                                </div>
                            </div>
                        @endif
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <!-- IP hiện tại -->
                    <div class="flex items-center justify-between p-3 {{ in_array($currentIP, $dsIP) ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-700/50' }} rounded-lg">
                    ...
                    <!-- WiFi hiện tại -->
                    <div class="flex items-center justify-between p-3 {{ $currentWiFi && in_array($currentWiFi, $dsWiFi) ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-gray-50 dark:bg-gray-700/50' }} rounded-lg">
                        <div class="flex items-center">
                        ...
                        <div class="text-right">
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $currentWiFi ?? 'Chưa kết nối' }}</span>
                    ...
                    <!-- Danh sách được phép -->
                    <details class="mt-4">
                        <summary class="text-sm font-medium text-blue-600 dark:text-blue-400 cursor-pointer hover:underline">
                        ...
                        <div class="mt-2 space-y-1">
                            @if(count($dsIP) > 0)
                                ...
                                <div class="flex flex-wrap gap-1 mt-1">
                                    @foreach($dsIP as $ip)
                                        <span class="inline-block text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full">{{ $ip }}</span>
                                    @endforeach
                                </div>
                            @endif
                            @if(count($dsWiFi) > 0)
                                ...
                                <div class="flex flex-wrap gap-1 mt-1">
                                    @foreach($dsWiFi as $wifi)
                                        <span class="inline-block text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-0.5 rounded-full">{{ $wifi }}</span>
                                    @endforeach
                                </div>
                            @endif
                        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Số giờ</th>
                                ...
                                @switch($item->trang_thai)
                                    ...
                                    @case('di_muon')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300">
                                            <i class="fas fa-exclamation-triangle mr-1"></i> Đi muộn ({{ $item->phut_di_muon }}p)
                                        </span>
                                        @break
                                    @case('ve_som')
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300">
                                            <i class="fas fa-exclamation-triangle mr-1"></i> Về sớm ({{ $item->phut_ve_som }}p)
                                        </span>
                                        @break
                                    @default
...
function showNotification(type, message) {
    ...
    notification.className = `fixed top-4 right-4 z-50 max-w-md w-full ${colors[type]} border rounded-xl p-4 shadow-lg transform transition-all duration-300 translate-x-full opacity-0`;
...
// =============================================
// LƯU WIFI VÀO LOCALSTORAGE ĐỂ TEST
// =============================================
// Mở console và chạy:
// localStorage.setItem('wifi_ssid', 'TP-Link_9FDC');
