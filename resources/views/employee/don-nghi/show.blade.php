        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <div class="mt-4 md:mt-0 flex items-center space-x-3">
                @if ($donNghi->trang_thai == 'cho_duyet')
                    ...
                    <form method="POST" action="{{ route('employee.don-nghi.huy', $donNghi->id) }}" class="inline"
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $statusColors[$donNghi->trang_thai] ?? 'gray' }}-100 text-{{ $statusColors[$donNghi->trang_thai] ?? 'gray' }}-800 dark:bg-{{ $statusColors[$donNghi->trang_thai] ?? 'gray' }}-900/30 dark:text-{{ $statusColors[$donNghi->trang_thai] ?? 'gray' }}-400">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ \Carbon\Carbon::parse($donNghi->ngay_bat_dau)->format('d/m/Y') }}</p>
                                <span
                                    class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($donNghi->ngay_bat_dau)->locale('vi')->dayName }}</span>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Đến ngày</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ \Carbon\Carbon::parse($donNghi->ngay_ket_thuc)->format('d/m/Y') }}</p>
                                <span
                                    class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($donNghi->ngay_ket_thuc)->locale('vi')->dayName }}</span>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Số ngày nghỉ</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ number_format($donNghi->so_ngay_nghi, 0) }} ngày</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Ngày tạo</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $donNghi->created_at->format('d/m/Y H:i') }}</p>
                                <span
                                    class="text-xs text-gray-400">{{ $donNghi->created_at->locale('vi')->diffForHumans() }}</span>
                            </div>
                        </div>
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        @elseif($donNghi->trang_thai == 'huy_bo')
                            <div class="text-center py-4">
                                <div
                                    class="w-12 h-12 bg-gray-100 dark:bg-gray-900/30 rounded-full flex items-center justify-center mx-auto mb-3">
                        ...
                        @else
                            <div class="text-center py-4">
                                <div
                                    class="w-12 h-12 bg-yellow-100 dark:bg-yellow-900/30 rounded-full flex items-center justify-center mx-auto mb-3">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Vui lòng chờ phê duyệt từ quản lý
                                </p>
                            </div>
                        @endif
                    </div>
                ...
                <div
                    ...
                    <div class="p-4">
                        @php
                            ...
                            // ĐÚNG: Chỉ tính tổng ngày của các đơn ĐÃ DUYỆT và KHÔNG THUỘC thai sản hoặc không lương
                            $soNgayDaNghi = App\Models\DonXinNghi::where('nguoi_dung_id', $donNghi->nguoi_dung_id)
                                ...
                                ->whereHas('loaiNghiPhep', function ($q) {
                                    $q->where('ten', 'not like', '%thai sản%')->where(
                                        'ten',
                                        'not like',
                                        '%không lương%',
                                    );
                        ...
                        <div class="mt-3">
                            ...
                            <div class="flex justify-between mt-1">
                                <span class="text-xs text-gray-500 dark:text-gray-400">Đã nghỉ
                                    {{ number_format($soNgayDaNghi, 0) }} ngày</span>
                                <span
                                    class="text-xs text-green-600 dark:text-green-400 font-medium">{{ number_format($soDuConLai, 0) }}
                                    ngày</span>
                            </div>
                        </div>
                    </div>
