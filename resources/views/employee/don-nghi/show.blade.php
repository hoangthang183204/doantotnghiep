                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $donNghi->loaiNghi->ten ?? 'Không xác định' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Trạng thái</p>
                            @php
                                $statusColors = ['cho_duyet' => 'yellow', 'da_duyet' => 'green', 'tu_choi' => 'red'];
                    ...
                    @if($donNghi->trang_thai != 'cho_duyet')
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ $donNghi->nguoiDuyet->hoSo->ho ?? '' }} {{ $donNghi->nguoiDuyet->hoSo->ten ?? $donNghi->nguoiDuyet->ten_dang_nhap ?? 'Chưa có' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Thời gian duyệt</p>
                            <p class="text-sm font-medium text-gray-900 dark:text-white">
                                {{ $donNghi->thoi_gian_duyet ? \Carbon\Carbon::parse($donNghi->thoi_gian_duyet)->format('d/m/Y H:i') : 'Chưa có' }}
                            </p>
                        </div>
                        @if($donNghi->ly_do_tu_choi)
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Lý do từ chối</p>
                                <p class="text-sm text-red-600 dark:text-red-400">{{ $donNghi->ly_do_tu_choi }}</p>
                            </div>
                        @endif
                    @else
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-green-600 dark:text-green-400">12 <span class="text-sm font-normal text-gray-500">ngày</span></p>
                        </div>
