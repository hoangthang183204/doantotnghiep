        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <div class="mt-4 md:mt-0">
                <a href="{{ route('employee.don-nghi.index') }}"
                    class="inline-flex items-center px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg transition-colors border-2 border-gray-300 dark:border-gray-600">
                ...
                <form method="POST" action="{{ route('employee.don-nghi.update', $donNghi->id) }}" class="space-y-6">
                            ...
                            <select name="loai_nghi_id" id="loai_nghi_id"
                                ...
                                @foreach ($loaiNghiPheps as $loai)
                                    <option value="{{ $loai->id }}"
                                        {{ old('loai_nghi_id', $donNghi->loai_nghi_phep_id) == $loai->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="ly_do" id="ly_do" rows="4"
                            ...
                            style="padding: 10px 14px;" placeholder="Nhập lý do xin nghỉ...">{{ old('ly_do', $donNghi->ly_do) }}</textarea>
                        @error('ly_do')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="ghi_chu" id="ghi_chu" rows="2"
                            ...
                            style="padding: 10px 14px;" placeholder="Ghi chú thêm (nếu có)...">{{ old('ghi_chu', $donNghi->ghi_chu) }}</textarea>
                        @error('ghi_chu')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Hàm tính toán số ngày nghỉ thực tế (loại trừ Thứ 7, Chủ Nhật)
            function tinhSoNgay() {
                if (ngayBatDau.value && ngayKetThuc.value) {
                    ...
                    if (start <= end) {
                        ...
                        while (current <= end) {
                            ...
                            if (dayOfWeek !== 0 && dayOfWeek !== 6) { // 0: CN, 6: T7
                                count++;
                            }
            ...
            // Chạy tính toán ban đầu nếu có dữ liệu
            tinhSoNgay();
        });
    </script>
@endpush
