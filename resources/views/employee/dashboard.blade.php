            ...
            <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
                ...
                <div class="flex-shrink-0">
                    <img src="{{ asset('storage/' . (auth()->user()->hoSo->anh_dai_dien ?? 'avatars/default.png')) }}" 
                         alt="Avatar" 
                         class="w-24 h-24 rounded-xl object-cover border-4 border-blue-100 dark:border-blue-900/30 shadow-md">
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        <span class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
                        ...
                        <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <span class="ml-2"><strong class="text-gray-900 dark:text-white">{{ number_format($soNgayLamViec) }}</strong> ngày làm việc</span>
                        </div>
                        <div class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                            <i class="fas fa-id-badge w-5 text-gray-400 dark:text-gray-500"></i>
                            <span class="ml-2">Mã NV: <strong class="text-gray-900 dark:text-white">{{ auth()->user()->hoSo->ma_nhan_vien ?? 'N/A' }}</strong></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-3">
                ...
                <div class="flex justify-between mt-1">
                    ...
                    <span class="text-xs text-red-500 dark:text-red-400">
                        <i class="fas fa-arrow-down mr-1"></i> {{ $soNgayThieu > 0 ? round(($soNgayThieu/$tongNgayTrongThang)*100) : 0 }}%
                    </span>
                </div>
            </div>
        </div>
                ...
                <div class="p-6">
                    <div class="text-center mb-4">
                        ...
                        <div class="mt-3">
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                                <div class="bg-green-500 h-2.5 rounded-full transition-all" style="width: {{ $tyLeTrenPhong }}%"></div>
                            </div>
                    ...
                    <div class="space-y-2 mt-4">
                        @foreach($topEmployees as $index => $emp)
                            <div class="flex items-center justify-between p-2 rounded-lg {{ $index == 0 ? 'bg-yellow-50 dark:bg-yellow-900/10' : ($index == 1 ? 'bg-gray-50 dark:bg-gray-700/30' : ($index == 2 ? 'bg-orange-50 dark:bg-orange-900/10' : '')) }}">
                                <div class="flex items-center space-x-3">
                                    <span class="w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold {{ $index == 0 ? 'bg-yellow-500 text-white' : ($index == 1 ? 'bg-gray-400 text-white' : ($index == 2 ? 'bg-orange-400 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300')) }}">
                                        {{ $index + 1 }}
                                    </span>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $emp['ho_ten'] }}</span>
                                </div>
                                <span class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ $emp['tong_cham_cong'] }} ngày</span>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="text-center p-4 rounded-xl bg-blue-50 dark:bg-blue-900/10">
                    <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ number_format($tongNgayChamCongNam) }}</p>
                ...
                <div class="text-center p-4 rounded-xl bg-yellow-50 dark:bg-yellow-900/10">
                    <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ number_format($tongDiTreNam) }}</p>
                ...
                <div class="text-center p-4 rounded-xl bg-cyan-50 dark:bg-cyan-900/10">
                    <p class="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{{ number_format($tongVeSomNam) }}</p>
                ...
                <div class="text-center p-4 rounded-xl bg-green-50 dark:bg-green-900/10">
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ number_format($tongNghiPhepNam) }}</p>
